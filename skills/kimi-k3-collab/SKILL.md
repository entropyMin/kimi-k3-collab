...
11. Non-Git execute is disabled by default. If the user explicitly authorizes `allow_non_git_execute=true` and the start result reports `isolation=single-writer`, stop all Codex writes in that directory until `await_k3_result` or `cancel_k3_job` releases the advisory lock. Do not describe non-Git execute as parallel-safe.

The MCP server owns Kimi's authenticated loopback-only WebSocket. The panel receives raw frames through a private app-only long-held host-bridge call, so it neither receives the bearer token nor needs loopback network access. Browser fallback uses an app-only, short-lived single-use ticket through a loopback gateway; do not expose or reconstruct a Kimi bearer URL. `await_k3_result` is the separate model-visible K3-to-Codex handoff. Neither path uses periodic status polling. If the host cannot render MCP Apps or make app-initiated tool calls, use the panel's Kimi Code browser fallback.
